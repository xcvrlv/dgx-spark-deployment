...
2. Launch `profiles/glm53-mtp-1.env`. Its single draft token is the low-memory
   ...
   performing a vocabulary-sized all-gather across TP4. The checkpoint's MTP
   experts are MXFP8; the image preserves that mixed-precision layer mapping
   and serves them through the SM121 Marlin fallback.
