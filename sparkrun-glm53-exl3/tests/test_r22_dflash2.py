...
def test_immutable_arm64_source_composition() -> None:
    ...
    for identity in (
        ...
        "803c857e70a85a6f8b7c75d50a0a3644e8620651",
    ...
    assert (
        'local-inference.vllm.composed.tree="803c857e70a85a6f8b7c75d50a0a3644e8620651"'
        in DOCKERFILE
    )
...
def test_exl3_overlay_is_fail_closed_and_uses_r22_b12x_abi() -> None:
    ...
    assert "Return graph-safe R22 dense-W4A16 scratch capacity" in source
    assert "including CUDA-graph decode shapes at or below 128 rows" in source
    ...
    assert "patch_b12x_glm_dsa_fp8_abi(root)" in source
    assert "_GLM_DSA_FP8_CACHE_RECORD_BYTES = 656" in source
    assert "else _GLM_DSA_FP8_CACHE_RECORD_BYTES" in source
    assert "vllm/v1/attention/backends/mla/b12x_mla_sparse.py" in DOCKERFILE
    assert "patch_rank_sliced_exl3_loaders(root)" in source
    ...
    assert "patch_nvidia_dcp_attention(root)" in source
    assert "2cf82bcdd17f658486c280b0acd23274f60bdf00" in source
    assert "vllm/models/deepseek_v32/attention.py" in DOCKERFILE
    assert "vllm/models/deepseek_v32/common/kernels.py" in DOCKERFILE
...
def test_four_spark_mtp3_runtime_contract() -> None:
    ...
    required = (
        ...
        "container: spark-vllm-glm53-exl3:r22-dflash2-sm121-v8",
        "min_nodes: 4",
        "max_nodes: 4",
        "tensor_parallel: 4",
...
def test_cluster_image_distribution_is_identity_checked() -> None:
    ...
    assert "r22-dflash2-sm121-v8" in BUILDER
