...
# loads and captures CUDA graphs. This must run on the host, not in the
# serving container: on GB10 the CPU and GPU share one physical pool, so every
# cached page is memory the model cannot have.
#
# The loop is intentionally unconditional. A Cached/MemAvailable threshold can
# leave several GiB resident and still let available memory collapse during
# graph capture; earlyoom then signals the highest-oom-score process, which is
# the vLLM worker. Lazy safetensors loading repopulates the cache page by page
# for the whole load window, so a single pre-start drop is not enough. The
# DS41 Engram table reads use O_DIRECT, so this cannot interfere with the SSD
# path's data; only file-backed pages (weights, caches, dentries) are dropped.
set -euo pipefail

duration="${CACHE_FLUSHER_DURATION_SECONDS:-${1:-5400}}"
interval="${CACHE_FLUSHER_INTERVAL_SECONDS:-60}"
lock_file="${CACHE_FLUSHER_LOCK_FILE:-${XDG_RUNTIME_DIR:-/tmp}/ds41-cache-flusher.lock}"
...
exec 9>"$lock_file"
if ! flock -n 9; then
  echo "FATAL: another DS41 cache flusher already holds $lock_file" >&2
  exit 1
fi
