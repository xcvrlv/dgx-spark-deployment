    ...
    def test_text_sources_have_linux_line_endings_and_no_trailing_space(self) -> None:
        ...
        for path in ROOT.rglob("*"):
            if not path.is_file() or path.suffix not in source_suffixes:
                continue
            if "external" in path.relative_to(ROOT).parts:
                continue
...
"""
    ...
    def test_sparkring_switch_launcher_uses_upstream_runtime_without_sircl(self) -> None:
        ...
        for fragment in (
            ...
            'MALLOC_ARENA_MAX=2',
            'oom={{.State.OOMKilled}} finished={{.State.FinishedAt}}',
    ...
    def test_sparkrun_glm53_exl3_recipe_uses_decode_only_graphs(self) -> None:
        ...
        flusher = (ROOT / "sparkrun-glm53-exl3/scripts/cache_flusher.sh").read_text()
        manager = (ROOT / "sparkrun-glm53-exl3/scripts/cache-flusher-cluster.sh").read_text()
        wrapper = (ROOT / "sparkrun-glm53-exl3/scripts/run-with-cache-flusher.sh").read_text()
        self.assertIn("name: juho-glm53-exl3", registry)
        self.assertIn("runtime: vllm-distributed", recipe)
        self.assertIn("container: spark-vllm-glm52-exl3:sparkring-switch-prefill-v2", recipe)
        self.assertIn("GLM53_INDEXER_WORKSPACE: rightsize", recipe)
        self.assertIn("model: /home/juho/.cache/huggingface/hub/models--davidsyoung--GLM-5.3-EXL3-TR3-3.42bpw", recipe)
        self.assertIn('gpu_memory_utilization: 0.91', recipe)
        self.assertNotIn('kv_cache_memory_bytes:', recipe)
        ...
        self.assertNotIn("--enforce-eager", recipe)
        self.assertIn("VLLM_MEMORY_PROFILER_ESTIMATE_CUDAGRAPHS: \"1\"", recipe)
        self.assertIn('"cudagraph_mode":"FULL_DECODE_ONLY"', recipe)
        self.assertIn('"cudagraph_capture_sizes":[4,8,12,16,20,24,28,32]', recipe)
        self.assertNotIn("FULL_AND_PIECEWISE", recipe)
        self.assertIn('\\"method\\":\\"mtp\\"', recipe)
        self.assertIn('\\"num_speculative_tokens\\":3', recipe)
        self.assertIn('\\"draft_tensor_parallel_size\\":4', recipe)
        self.assertIn('\\"quantization\\":\\"exl3\\"', recipe)
        self.assertIn('\\"moe_backend\\":\\"b12x\\"', recipe)
        self.assertIn('\\"attention_backend\\":\\"B12X_MLA_SPARSE\\"', recipe)
        self.assertIn("shm_size: 32gb", recipe)
        self.assertIn("unset VLLM_PREFIX_CACHE_RETENTION_INTERVAL", recipe)
        self.assertNotIn('VLLM_PREFIX_CACHE_RETENTION_INTERVAL: ""', recipe)
        self.assertIn("OMP_NUM_THREADS: \"8\"", recipe)
        self.assertIn("VLLM_EXL3_PREFILL_CAPACITY: \"4096\"", recipe)
        self.assertIn("VLLM_EXL3_TRELLIS_MIN_M: \"1\"", recipe)
        self.assertIn("VLLM_EXL3_TRELLIS_MAX_M: \"64\"", recipe)
        self.assertIn("VLLM_EXL3_TRELLIS_BLOCK_M: \"8\"", recipe)
        self.assertIn("VLLM_EXL3_PREFILL_TRELLIS: \"1\"", recipe)
        self.assertIn("VLLM_EXL3_PREFILL_BLOCK_M: \"32\"", recipe)
        self.assertIn("VLLM_EXL3_PREFILL_CHUNK: \"128\"", recipe)
        self.assertIn("VLLM_USE_V2_MODEL_RUNNER: \"1\"", recipe)
        self.assertIn("VLLM_USE_B12X_SPARSE_INDEXER: \"1\"", recipe)
        self.assertIn("VLLM_B12X_MLA_SPEC_EXTEND_AS_DECODE: \"1\"", recipe)
        self.assertIn("VLLM_B12X_MLA_CKV_GATHER: \"1\"", recipe)
        self.assertIn("VLLM_DCP_GLOBAL_TOPK: \"1\"", recipe)
        self.assertIn("VLLM_DCP_SHARD_DRAFT: \"1\"", recipe)
        self.assertIn("VLLM_SPARSE_INDEXER_MAX_LOGITS_MB: \"256\"", recipe)
        self.assertIn("NCCL_MAX_NCHANNELS: \"4\"", recipe)
        self.assertIn("NCCL_MIN_NCHANNELS: \"4\"", recipe)
        self.assertIn("--async-scheduling", recipe)
        self.assertIn('B12X_W4A16_TC_DECODE: "1"', recipe)
        self.assertIn("--reasoning-parser glm45", recipe)
        self.assertIn("--tool-call-parser glm47", recipe)
        self.assertIn("--enable-auto-tool-choice", recipe)
        self.assertIn("ONLINE_QUANT: exl3-b6", recipe)
        self.assertIn("VLLM_EXL3_ONLINE_CACHE_MODE: readwrite", recipe)
        self.assertIn(".exl3-online-k6", recipe)
        self.assertIn("--quantization-config", recipe)
        self.assertIn('"shared_experts":{"weight":"mxfp8"}', recipe)
        self.assertIn("--generation-config vllm", recipe)
        self.assertIn("--hf-overrides", recipe)
        self.assertIn("FFFSSSFSSSFSSS", recipe)
        self.assertNotIn("--enable-decode-aware-prefill", recipe)
        self.assertNotIn("VLLM_ADAPTIVE_SPEC_DEPTHS", recipe)
        self.assertNotIn("mods:", recipe)
        self.assertIn("auto_remove: false", recipe)
        ...
        self.assertIn("sudo -n tee /proc/sys/vm/drop_caches", flusher)
        self.assertIn('CACHE_FLUSHER_INTERVAL_SECONDS:-60', flusher)
        self.assertIn("flusher: starting, unconditional", flusher)
        self.assertNotIn("Cached", flusher.split("drop_page_cache()", maxsplit=1)[1])
        self.assertIn("installing cache flusher", manager)
        self.assertIn("refresh the load window", manager)
        self.assertIn("stopping the nodes already started", manager)
        self.assertIn("cache_flusher.sh", manager)
        self.assertIn('bash "$manager" start "$hosts"', wrapper)
        self.assertIn("FATAL: not every node has an active cache flusher", wrapper)
        self.assertLess(
            wrapper.index('bash "$manager" start "$hosts"'),
            wrapper.index('if ! sparkrun run "${args[@]}"'),
        )
