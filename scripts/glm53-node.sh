...
PY
  ...
  docker run --rm --entrypoint python3 "$IMAGE" -c \
    'from vllm.model_executor.layers.quantization.modelopt import ModelOptMixedPrecisionConfig as C; q=object.__new__(C); q.quantized_layers={"language_model.model.layers.45.mlp.experts":{"quant_algo":"MXFP8"}}; p="model.layers.45.mtp_block.mlp.experts"; resolved=q._resolve_quant_algo(p); assert resolved == "MXFP8", (p, resolved)' \
    >/dev/null
