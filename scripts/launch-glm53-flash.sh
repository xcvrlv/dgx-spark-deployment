...
VLLM_UMA_USE_MEM_AVAILABLE="${VLLM_UMA_USE_MEM_AVAILABLE:-0}"
...
for boolean_name in ENFORCE_EAGER VERIFY_CUDA_GRAPHS MTP_USE_LOCAL_ARGMAX_REDUCTION Q40_ENABLED UMA_DROP_CACHES_BEFORE_START VLLM_UMA_USE_MEM_AVAILABLE; do
  [[ "${!boolean_name}" =~ ^[01]$ ]] || {
...
remote_node() {
  ...
  local assignments=(
    ...
    "VLLM_UMA_USE_MEM_AVAILABLE=$VLLM_UMA_USE_MEM_AVAILABLE"
