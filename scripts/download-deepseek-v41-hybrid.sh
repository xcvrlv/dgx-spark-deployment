#!/usr/bin/env bash
# Standalone head-Spark setup: authenticate, download hybrid, verify, copy peers.
# The Python implementation below is embedded; no repository checkout is required.
set -euo pipefail

hub_dir="${HF_HUB_CACHE:-${HF_HOME:-$HOME/.cache/huggingface}/hub}"
model_dir="${HYBRID_MODEL_DIR:-$hub_dir/DeepSeek-V4.1-Flash-MXFP4-FP4-Engram}"
venv_dir="${DOWNLOAD_VENV:-$HOME/.local/share/deepseek-v41-download/venv}"
peer_hosts="${SPARK_PEERS:-192.168.0.2,192.168.0.3,192.168.0.4}"

command -v python3 >/dev/null || { echo 'Install python3 and python3-venv first.' >&2; exit 1; }
command -v rsync >/dev/null || { echo 'Install rsync first (also required on peers).' >&2; exit 1; }
command -v ssh >/dev/null || { echo 'Install openssh-client first.' >&2; exit 1; }
if [[ ! -x "$venv_dir/bin/python" ]]; then
  python3 -m venv "$venv_dir"
fi
"$venv_dir/bin/python" -m pip install 'huggingface_hub>=1,<2'

# Login prompts securely; the token is never embedded in this file or sent to peers.
# Existing saved credentials or HF_TOKEN are reused.
if ! "$venv_dir/bin/hf" auth whoami >/dev/null 2>&1; then
  echo 'Log in with your Hugging Face read token. Git credential storage is unnecessary.'
  "$venv_dir/bin/hf" auth login
fi

echo "Model destination: $model_dir"
echo "Peers: $peer_hosts (same absolute destination path)"
echo 'Allow approximately 390 GiB free per host. Rerun this script to resume.'
"$venv_dir/bin/python" - --model-dir "$model_dir" --copy --hosts "$peer_hosts" "$@" <<'DEEPSEEK_HYBRID_PYTHON'
#!/usr/bin/env python3
